...
export default function Dashboard() {
  ...
  // Custom hooks
  const { routines, todayData, activeSession: currentSession } = useDashboardQueries();
  const { data: routineExercises = [] } = useRoutineExercises(activeSession?.routine_id || null);
  const { data: setRecords = [] } = useSetRecords(activeSession?.id || null);
  ...
  const handleStartWorkout = (routineId: string) => {
    ...
    setActiveSession({ id: '', start_time: new Date().toISOString(), end_time: null, duration_minutes: null, routine_id: routineId });
    setSessionStartTime(new Date());
  };

  const handleEndWorkout = () => {
    if (!activeSession || !sessionStartTime) return;
    endWorkoutMutation.mutate({ activeSession, sessionStartTime });
  ...
  const handleAddSet = (exerciseId: string, reps: number, weight: number) => {
    ...
    addSetMutation.mutate({ sessionId: activeSession.id, exerciseId, reps, weight, setRecords });
  };
